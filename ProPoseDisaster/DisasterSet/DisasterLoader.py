...
class DisasterDatasetLoader():
    def __init__(self, rootpath):
        ...
        #bind box
        print('calculate binding box position...')
    ...
    def get_all_sequences(self):
        ...
        '''
        subfolders = [ name for name in os.listdir(self.rootpath) if os.path.isdir(os.path.join(self.rootpath, name))]
        return subfolders
    ...
    def get_all_images(self):
        ...
        for s in self.sequences:
            img_path = []
            name = []
            path = os.path.join(self.rootpath, s, 'images')
            # root path, sub-file-folder, file-list
            for root, dirs, file_names in os.walk(path):
                for file_name in file_names:
                    name.append(file_name)
                    img_path.append(os.path.join(self.rootpath, s, 'images', file_name))
            names.append(name)
    ...
    def get_cam_filepaths(self):
        ...
        for s in self.sequences:
            cam_filepath = []
            path = os.path.join(self.rootpath, s, 'camera')
            for root, dirs, file_names in os.walk(path):
                for file_name in file_names:
    ...
    def get_uv_joint_filepaths(self):
        ...
        for s in self.sequences:
            uv_joint_filepath = []
            path = os.path.join(self.rootpath, s, '2Djoint')
            for root, dirs, file_names in os.walk(path):
                for file_name in file_names:
    ...
    def get_all_mask(self):
        ...
        for s in self.sequences:
            mask_path = []
            mask_name = []
            path = os.path.join(self.rootpath, s, 'mask')
            for root, dirs, file_names in os.walk(path):
                for file_name in file_names:
                    mask_path.append(os.path.join(self.rootpath, s, 'mask', file_name))
    ...
    def get_focal_length(self):
        ...
        '''
        for s in self.sequences:
            path = os.path.join(self.rootpath, s, 'cam_info.txt')
            with open(path, "r", encoding="utf-8") as file:
                lines = file.readlines()
            data = lines[1].split(',')
            FOV = float(data[0])
            SENSOR_WIDTH = float(data[1][:-1])
    ...
    def get_uv_joints(self):
        ...
        joint_visibles = []
        uv_joints = []
        for sequence_ in self.uv_joint_filepaths:
            joint_visible_in_all_file = []
            uv_joint_in_all_file = []
            for path in sequence_:
    ...
    def get_beta(self):
        ...
        for s in self.sequences:
            beta = []
            df = pd.read_csv(os.path.join(self.rootpath, s, s+'-beta.csv'), header = None, names=['value'])
    ...
    def get_theta_joings(self): 
        ...
        for s in self.sequences:
            ...
            joint_loc = []
            gender_tran = []
            df = pd.read_csv(os.path.join(self.rootpath, s, s+'-joint.csv'))
            for index, row in df.iterrows():
    ...
    def get_binding_box(self):
        ...
        for mask_path in self.mask_paths:
            bb = []
            i += 1
            print('calculating',i, '-th sequence')
            for path in mask_path:
                j += 1
                print('calculating',j, '-th image in ',i, '-th sequence')
...
# with open("./DisasterSet/allset/LV_000006/cam_info.txt", "r", encoding="utf-8") as file:
#     lines = file.readlines()
